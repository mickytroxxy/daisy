...
const AIServiceAdvisor: React.FC = () => {
  ...
  const getRecommendation = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error("AI Assistant is currently offline. Please contact support.");
      }
      ...
      if (cat.includes('print')) icon = 'Print';
      if (cat.includes('connectivity')) icon = 'Connectivity';
      if (cat.includes('solar')) icon = 'Energy';
      if (cat.includes('apple')) icon = 'Apple';
      if (cat.includes('super')) icon = 'Supercomputer';
  ...
  return (
    <div className="bg-[#003366] rounded-[2.5rem] p-8 lg:p-12 shadow-2xl border border-white/10 relative overflow-hidden h-full flex flex-col justify-center">
      ...
      <div className="relative z-10">
        ...
        <p className="text-slate-400 mb-8 font-light text-sm">Describe your infrastructure needs and let our AI suggest the right enterprise solution.</p>
        ...
        <div className="space-y-4">
          <textarea
            ...
            placeholder="e.g., We need to transition our design team to MacBooks and secure a solar backup for our server room..."
            className={`w-full bg-white/5 border ${error ? 'border-red-500/50' : 'border-white/10'} rounded-2xl p-4 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#76bc21] transition-all resize-none h-32 text-sm leading-relaxed`}
          />
          ...
          <button
            ...
            className="w-full bg-[#76bc21] text-white font-black py-4 rounded-xl hover:bg-[#65a31c] transition-all disabled:opacity-50 flex items-center justify-center space-x-2 shadow-lg shadow-[#76bc21]/20 active:scale-95"
          >
            {loading ? (
              <>
        ...
        {recommendation && (
          <div className="mt-8 p-6 bg-white/5 rounded-2xl border border-[#76bc21]/30 animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center space-x-4 mb-4">
              <div className="w-12 h-12 bg-[#76bc21] rounded-xl flex items-center justify-center text-white shadow-lg shadow-[#76bc21]/20">
              ...
              <div>
                <div className="text-[#76bc21] text-[10px] font-black uppercase tracking-widest">Target Solution</div>
                <div className="text-white font-black text-xl leading-none mt-1">{recommendation.category}</div>
              </div>
            </div>
            <p className="text-slate-300 font-light mb-6 text-sm leading-relaxed">
              {recommendation.reasoning}
            </p>
            <a href="#/services" className="inline-flex items-center text-[#76bc21] font-bold hover:underline text-sm group">
